...
const statusTabs = ['All', 'Watching', 'Plan to Watch', 'Completed', 'Dropped'] as const;
...
export default function WatchlistPage() {
  ...
  useEffect(() => {
    apiClient.get<{ results: WatchlistItem[] }>('/watchlist')
      .then((res) => setItems(res.results || []))
      .catch((err) => setError(err.message))
      .finally(() => setLoading(false));
  }, []);

  const filtered = activeTab === 'All' ? items : items.filter((item) => {
    ...
    return item.status === map[activeTab];
  });
        ...
        <div className="px-[40px] py-8 max-w-[1800px] mx-auto">
          <div className="mb-10 space-y-6">
            <div className="flex items-end justify-between">
              <div>
                <h2 className="font-display text-display-xl text-on-surface mb-2">Watchlist Archive</h2>
                <p className="font-body text-body-md text-on-surface-variant">Manage and track your cinematic journeys.</p>
              </div>
